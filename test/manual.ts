...
async function main(){
    ...
    console.log('big adds', result.filter(commit => commit.changes.some(c => c.added > 1000)));
    console.log('big removes', result.filter(commit => commit.changes.some(c => c.removed > 1000)));
